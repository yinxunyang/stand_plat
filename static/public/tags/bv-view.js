define([
        ...
        mounted: function () {
            ...
            $(this.$el).on('click', 'input[type=checkbox]', function () {
                ...
                if (position === 'floor') {
                    ...
                    vm.checkSelected($(this).closest('.view-property'));
        ...
        methods: {
            ...
            init: function () {
                ...
                for (var i=0; i<this.properties.length; i++) {
                    if (!util.isEmpty(this.properties[i].houseList)) {
                        for (var j=0; j<this.properties[i].houseList.length; j++) {
                            var cellNo = this.properties[i].houseList[j].cellNo;
                            var floorNo = this.properties[i].houseList[j].floorNo;
                            if (!cellFloorHouses[this.properties[i].propertyCode + '-' + floorNo]) {
                                cellFloorHouses[this.properties[i].propertyCode + '-' + floorNo] = {};
                            }
                            if (!cellFloorHouses[this.properties[i].propertyCode + '-' + floorNo][cellNo]) {
                                cellFloorHouses[this.properties[i].propertyCode + '-' + floorNo][cellNo] = [];
                            }
                            cellFloorHouses[this.properties[i].propertyCode + '-' + floorNo][cellNo].push(this.properties[i].houseList[j]);
                        }
                    }
                    for (var f=0; f<this.properties[i].cellList.length; f++) {
                        var cellNo = this.properties[i].cellList[f].cellNo;
                        var maxHouse = 0;
                        for (var pf in cellFloorHouses) {
                            if (util.startsWith(pf, this.properties[i].propertyCode + '-')) {
                        ...
                        cellHouseNum[this.properties[i].propertyCode + '-' + cellNo] = maxHouse;
                    }
                    for (var f=0; f<this.properties[i].floorList.length; f++) {
                        var floorNo = this.properties[i].floorList[f].floorNo;
                        var maxHouse = 0;
                        for (var p in cellFloorHouses[this.properties[i].propertyCode + '-' + floorNo]) {
                            if (maxHouse < cellFloorHouses[this.properties[i].propertyCode + '-' + floorNo][p].length) {
                                maxHouse = cellFloorHouses[this.properties[i].propertyCode + '-' + floorNo][p].length;
                            }
                        }
                        floorHouseNum[this.properties[i].propertyCode + '-' + floorNo] = maxHouse;
                    }
                    /*var cells = [];
                    if (!util.isEmpty(this.properties[i].cellList)) {
                        for (var j=0; j<this.properties[i].cellList.length; j++) {
                            cells.push(this.properties[i].cellList[j].cellNo);
                        }
                    }
                    propertyCells[this.properties[i].propertyCode] = cells;*/
                    propertyCells[this.properties[i].propertyCode] = null;
                }
            ...
            checkSelected: function ($property) {
                // 先计算选择的房屋
                this.selected.houses = [];
                this.$nextTick(function () {
                    var vm = this;
                    $('.house [data-position=house]:checked', this.$el).each(function () {
                        vm.selected.houses.push($(this).attr('data-id'));
                    });
                });
            },
    ...
    vue.component('bv-buildings', {
        ...
        mounted: function () {
            var vm = this;
            $(this.$el).on('click', '.building', function () {
                ...
                var selected = [];
                $('.building.selected', vm.$el).each(function () {
        ...
        /****** 模板定义 ******/
        template: util.template('bv-buildings')
    });
});