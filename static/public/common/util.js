define('util', ['vue', 'jquery', 'Const', 'text!template/tags.html', 'moment', 'md5'], function(vue, $, Const, template, moment, md5) {
    /******************************************* 自定义异常 *********************************************************/
    // 转base64用到的异常
    ...
    return {
        ...
        random: function (min, max) {
            if (this.type(min) === 'undefined' || this.type(max) == 'undefined') {
                return this.formatNumber(Math.random(), 2);
            }
        ...
        // 取url路径
        path: function () {
            if (Const.route.hash === false) {
                /*if (Const.route.baseLocation) {
                    return (location.pathname + location.search).replace(Const.route.baseLocation, '');
                }*/
                return location.pathname + location.search;
            }
            var h = location.hash;
            if (h) {
                if (this.startsWith(h, Const.route.hash)) {
                    return h.substring(Const.route.hash.length);
        ...
        // 左补零/其他字符
        fill: function (s, c, l) {
            if (this.type(l) === 'undefined') {
                l = c;
                c = '0';
            }
            var len = ('' + s).length;
            return (Array(l > len ? l - len + 1 || 0 : 0
            ).join(c || 0) + s);
        },
        // url编码
        encode: function(s) {
        ...
        url: function(url, type) {
            ...
            if (type && (type === 'html' || type === 'static')) {
                if (this.inSimple()) {
                    // 普通单页面
                    if (!this.startsWith(url, '/')) {
                        url = '/' + url;
                    }
                    if (Const.route.virtualLocation && !this.startsWith(url, Const.route.virtualLocation)) {
                        url = Const.route.virtualLocation + '/' + Const.init.appCode + url;
                    }
                    ...
                    if (Const.route.virtualLocation && !this.startsWith(url, Const.route.virtualLocation)) {
                        url = Const.route.virtualLocation + '/' + Const.init.appCode + Const.route.htmlLocation + url;
                    }
                }
                /*if (Const.route.baseUrl && this.inSimple()) {
                    return Const.route.baseUrl + Const.route.baseLocation + Const.route.versionLocation + Const.route.htmlLocation + url;
                }*/
            ...
            if (this.endsWith(url, '.js')) {
                if (this.inSimple()) {
                    ...
                    if (Const.route.virtualLocation && !this.startsWith(url, Const.route.virtualLocation)) {
                        url = Const.route.virtualLocation + '/' + Const.init.appCode + Const.route.scriptLocation + url;
                    }
                }
        ...
        data: function(data) {
            ...
            return data && data.head ? data.body : data;
        },
        /******************************************* 弹窗相关 *********************************************************/
        // 打开弹出窗口，不支持多级窗口级联打开
        // 参数
                ...
                if (param.type) {
                    if (param.type === 'delete') {
                        ...
                        if (param.from && param.from === 'form') {
                            ...
                            param.url = param.url || Const.url.form.del;
                            if (param.vm) {
        ...
        layout: function($element) {
            if ($element) {
                if (this.type($element) === 'number') {
                    if ($element >= 1800) {
                        return 'form-layout-6';
                    } else if ($element >= 1600) {
                        return 'form-layout-5';
                    } else if ($element >= 1200) {
                        return 'form-layout-4';
                    } else if ($element >= 900) {
                        return 'form-layout-3';
                    } else if ($element >= 600) {
                        return 'form-layout-2';
                    }
                    return 'form-layout-1';
                }
                if ($($element).closest('.bv-modal').length === 1) {
                    return 'modal';
                } else if ($($element).closest('.modal').length === 1) {
                    return 'modal';
                } else if ($($element).closest('.bv-body').length === 1 || this.inSimple()) {
                    return 'body';
                }
            } else {
                if ($('#modal').is(':visible')) {
        ...
        isCurrent: function($element) {
            if ($($element).closest('.bv-content').is('.tabs-cascade') && $($element).closest('.tab-pane').length === 1) {
                return $($element).closest('.tab-pane').is('.active');
            }
            return $($element).closest('.bv-content').attr('data-active') === 'true';
        },
        // 是否在标签页中
        inTabContent: function ($element) {
            return $element && $($element).closest('.tab-content-inline').length === 1;
        },
        inSimple: function () {
            return $('.bv-simple').length === 1;
        },
        // 刷新当前标签页
        reload: function () {
        ...
        clone: function(value, origin, onlyUpdate) {
            if (value !== undefined && origin !== undefined) {
                if (this.type(origin) === 'string') {
                    ...
                    this.warn('请使用util.set来设置vm属性值');
                    this.set(value, origin, onlyUpdate);
                } else {
                    for (var p in origin) {
...
//            this.scroll($element);
        ...
        // 页面跳转（局部）
        redirect: function(url, type, callback) {
            if (!type && this.type(url) === 'string') {
                ...
                // type -> callback
                this.replace($target, url, type);
                if (Const.route.virtualLocation && this.startsWith(url, Const.route.virtualLocation)) {
                    url = url.replace(Const.route.virtualLocation, '');
                }
                Const.init.path = url;
                if ((param.popstate === undefined || param.popstate !== false)) {
                    /*if (Const.route.baseLocation) {
                        url = Const.route.baseLocation + Const.route.versionLocation + (url && url !== '/' ? (Const.route.hash !== false ? '/' + Const.route.hash + '/' : '') + url : '/');
                    }*/
                    if (param.ignore) {
                        if (param.back === false) {
        ...
        // 根据body拼装html代码
        html: function (css, body, callback) {
            if (this.startsWith(css, '/')) {
                css = '/public' + css;
            }
            this.get({
                url: css,
                dataType: 'text',
                success: function (res) {
                    callback.call(null, '<html><head><style>' + res + '</style></head><body>' + body + '</body></html>');
                }
            })
        },
        // 获取元素宽度
        ...
        /*wrap: function ($element) {
            ...
            var isWrap = ($('.label-text', $element).height() > this.toNumber(lineHeight.replace('px', '')));
            if (isWrap && !$($element).hasClass('wrap')) {
                $($element).addClass('wrap');
            } else if (!isWrap && $($element).hasClass('wrap')) {
        ...
        mainTabs: function(config, titles, data) {
            if (this.type(config) === 'string') {
                ...
                if (config.entity) {
                    ...
                    var titles = [];
                    titles.splice(0, 0, {
                        text: config.name || config.name_origin
                    });
                    var menuNode = config;
                    while (menuNode.level > 0) {
                        menuNode = menuNode.getParentNode();
                        titles.splice(0, 0, {
                            text: menuNode.name || menuNode.name_origin
                        });
                    }
                    this.mainTabs({
        ...
        keySwitch: function () {
            if (!window.keySwitchInited) {
                window.keySwitchInited = true;
                ...
                // $(document).on('keydown', 'form', disableEnterDown);
                var util = this;
                $(document).on('keyup next', 'form', function (event) {
                    if (event.type === 'next' || event.keyCode == 13) {
                        var $element = $(event.target);
                        if ($element.is(event.type === 'keyup' && '[data-provide="typeahead"]')) {
                            return true;
                        }
                        if ($element.is('input,button,select')) {
                            var $inputs = $('input,button,textarea,select', this).filter(':visible:enabled').filter(function() {
                                if ($(this).attr('readonly') === 'true' || $(this).attr('readonly') === 'readonly' || $(this).parent().is('.input-group-btn')) {
                                    return false;
                                }
                                var tabindex = $(this).attr('tabindex');
                                if (tabindex === undefined || util.toNumber(tabindex) >= 0) {
                                    return true;
                                }
                                return false;
                            });
                            ...
                            if ($inputs.eq(index + 1).is('button:not(.dropdown-toggle)')) {
                                $inputs.eq(index + 1).click();
                            } else {
                                $inputs.eq(index + 1).select();
                            }
                        }
                        /*var buttons = "button,reset,submit"; //button格式
                        if (element.nodeName == "INPUT" || element.nodeName == "SELECT" || $(element).is('.dropdown-toggle')) {
                            //获取缓存的页面input集合
        ...
        validateInit: function($element, validateInline) {
            ...
            if (!$($form).data('jqv') && $($form).is('form')) {
                ...
                this.require('validation', function () {
        ...
        call: function (param) {
            ...
            if (param.vm.execute && this.type(param.vm.execute) === 'function') {
                return param.vm.execute(param);
            } else {
                ...
                if (tagName) {
                    if (tagName === 'bv-table') {
                        ...
                        if (param.get) {
                            if (param.get === 'count') {
                                return param.vm.pager.rowCount || 0;
                            }
                        }
                        if (param.query) {
                        ...
                        if (param.appendRows) {
                            ...
                            if (param.showProgress) {
                                t = param.vm.handleProgress();
                            }
                            param.vm.rows.push.apply(param.vm.rows, param.appendRows);
                            param.vm.pager.rowCount = param.vm.pager.rowCount + param.appendRows.length;
                        ...
                        if (param.type === 'calc') {
                            ...
                            if (!param.type || param.type === 'refresh' || param.type !== 'load' || !param.vm.inited) {
                                if (this.type(param.page) !== 'undefined') {
                                    param.vm.pager.currentPage = param.page;
                                }
                                param.vm.refresh();
                            }
                        }
                        ...
                        if (param.get) {
                            // 获取数据
                            if (param.get === 'labels') {
                                return param.vm.localLabels;
                            } else if (param.get === 'datasets') {
                                return param.vm.localDatasets;
                            } else if (param.get === 'both') {
                                return {
                                    labels: param.vm.localLabels,
                                    datasets: param.vm.localDatasets
                                };
                            }
                        }
                        if (param.type) {
                        ...
                        if (param.labels) {
                            param.vm.localLabels = param.labels;
                        }
                        if (param.datasets) {
                            param.vm.localDatasets = param.datasets;
                        }
                        param.vm.refresh(param.labels, param.datasets);
                        ...
                        if (param.method) {
                            if (param.method === 'upload') {
                                param.vm.ajaxUpload(undefined, undefined, param.onFileUploaded);
                            }
                        }
                        if (param.fileType) {
                            param.vm.fileType = param.fileType;
                        }
                        if (param.subType) {
                            param.vm.subType = param.subType;
                        }
                    } else if (tagName === 'bv-wizard') {
                        if (param.direction) {
                            param.vm.active(param.event, param.direction, param.enable);
                        }
                    } else if (tagName === 'bv-static') {
                        if (this.type(param.text) !== 'undefined') {
                            param.vm.initVal(param.text);
                        }
                    } else if (tagName === 'bv-editor') {
                        if (param.type) {
                            if (param.type === 'getContent') {
                                return $('#container', param.vm.$el).summernote('code');
                            } else if (param.type === 'setContent') {
                                $('#container', param.vm.$el).summernote('code', param.content);
                            }
                        }
                    } else if (tagName === 'bv-building') {
                        if (param.selected === 'house') {
                            return param.vm.selected.houses;
                        }
                    } else if (tagName === 'bv-buildings') {
                        if (param.selected === 'building') {
                            return param.vm.selected;
                        }
                    } else if (tagName === 'bv-grant') {
                        if (param.left) {
                            param.vm.entity[param.vm.left] = param.left;
                        }
                        if (param.right) {
                            param.vm.entity[param.vm.right] = param.right;
                        }
                    } else if (tagName === 'bv-select') {
                        if (param.initParamList) {
                            param.vm.initParamList = param.initParamList;
                            this.initSelectData(param.vm, 'select', 'load');
                        }
                    }
                    if (param.attr) {
                        if (param.attr.title) {
                        ...
                        if (this.type(param.attr.disabled) !== 'undefined') {
                            if (tagName === 'bv-select') {
                                $('button.dropdown-toggle', param.vm.$el).attr('disabled', param.attr.disabled);
                            } else if (tagName === 'bv-button') {
                                param.vm.disabled = param.attr.disabled;
                                delete param.attr.disabled;
                            }
                        }
                    ...
                    if (param.value !== undefined) {
                        ...
                        param.vm.entity[param.vm.name] = param.value;
                        if (tagName === 'bv-select') {
                            $('select', param.vm.$el).selectpicker('val', param.value);
                        }
                        // param.vm.entity[param.vm.name] = param.value;
                    }
                ...
                if (param.message) {
                    if (Const.init.show.type === 'alert') {
                     ...
                     param.$element.validationEngine('showPrompt', param.message, 'error', param.position || 'topRight', true);
                        ...
                        this.require('toastr', function (toastr) {
        ...
        capture: function (type) {
            if (type && type === 'close') {
                if (Const.global.captureDialog) {
                    Const.global.captureDialog.get_actionType("closeSignal");
                    Const.global.captureDialog = null;
                }
                if (Const.global.captureSocket) {
                    Const.global.captureSocket.close();
                    Const.global.captureSocket = null;
                }
                if (Const.vm.captureVm) {
                    Const.vm.captureVm = null;
                }
            }
        },
        // 获取guid
        ...
        validateMix: function(validate) {
            ...
            if (validate.idCard !== undefined) {
                if (validateStr) {
                    validateStr += ',';
                }
                if (this.type(validate.idCard) === 'boolean') {
                    validateStr += 'idCard';
                } else if (this.type(validate.idCard) === 'string') {
                    validateStr += 'idCard[' + validate.idCard + ']';
                }
            }
        ...
        validateUpdate: function (vm, validate) {
            if (vm) {
                this.validateHide(vm.$el);
                vue.delete(vm.attr, 'data-validation-engine');
                for (p in vm.attr) {
                    if (this.startsWith(p, 'data-errormessage')) {
                        vue.delete(vm.attr, p);
                    }
                }
                if (validate) {
                    var result = this.validateMix(validate);
                    for (var p in result) {
                        vue.set(vm.attr, p, result[p]);
                    }
                }
                var util = this;
                vm.$nextTick(function () {
                   util.validate($('[data-validation-engine]', vm.$el), true);
                });
            }
        },
        validateHide: function ($element) {
            $($element).validationEngine('hide');
        },
        transOrder: function (orderList, orders, columns) {
            if (!orderList) {
                orderList = [];
            }
            if (orders) {
        ...
        // 滚动条美化
        scroll: function($element, async) {
            if (!Const.global.isMobile && !Const.global.isIe) {
                if (!$element) {
                    $element = $(Const.vm.current.$el);
                }
                if (async) {
                    if (async === 'destroy') {
                        this.require('nicescroll', function () {
                            $($element).width('auto').getNiceScroll().remove();
                        });
                    } else {
                        var util = this;
                        vue.nextTick(function () {
        ...
        initDefault: function(vm) {
            ...
            // 处理默认排序
            if (vm.orders) {
                this.transOrder(vm.orderList, vm.orders, vm.columns);
...
//                        $.extend(Const.params, util.data(data));
        ...
        require: function (type, callback) {
            if (type) {
                if (type === 'toastr') {
                    var util = this;
                    require([Const.init.baseResourcePath + '/plugin/toastr/toastr.min.js', 'css!' + Const.init.baseResourcePath + '/plugin/toastr/toastr.css'], function (toastr) {
                        if (util.type(callback) === 'function') {
                            callback.call(null, toastr);
                        }
                    });
                } else if (type === 'validation') {
                    require(['css!' + Const.init.baseResourcePath + '/plugin/validation/validationEngine.jquery.min.css', Const.init.baseResourcePath + '/plugin/validation/jquery.validationEngine.min.js',
                            Const.init.baseResourcePath + '/plugin/validation/languages/jquery.validationEngine-zh_CN.js'], function () {
                        callback.call(null);
                    });
                } else if (type === 'nicescroll') {
                    require([Const.init.baseResourcePath + '/plugin/jquery-nicescroll/jquery.nicescroll.min.js?v=1805281649'], function () {
                        callback.call(null);
                    });
                } else if (type === 'datetimepicker') {
                    require(['css!' + Const.init.baseResourcePath + '/plugin/bootstrap-datetimepicker/css/bootstrap-datetimepicker.min.css', Const.init.baseResourcePath + '/plugin/bootstrap-datetimepicker/js/bootstrap-datetimepicker.min.js'], function () {
                        callback.call(null);
                    });
                } else if (type === 'gallery') {
                    require(['css!' + Const.init.baseResourcePath + '/plugin/lightGallery/css/lightgallery.min.css', 'mousewheel', Const.init.baseResourcePath + '/plugin/lightGallery/js/lightgallery.js'], function (gallery) {
                        require(['css!' + Const.init.baseResourcePath + '/plugin/cropper/cropper.min.css',  Const.init.baseResourcePath + '/plugin/lightGallery/js/lg-fullscreen.min.js',
                            Const.init.baseResourcePath + '/plugin/lightGallery/js/lg-zoom.min.js', Const.init.baseResourcePath + '/plugin/lightGallery/js/lg-pager.min.js',
                            Const.init.baseResourcePath + '/plugin/lightGallery/js/lg-thumbnail.min.js', Const.init.baseResourcePath + '/plugin/cropper/cropper.min.js'], function () {
                            callback.call(null);
                        });
                    });
                } else if (type === 'typeahead') {
                    require([Const.init.baseResourcePath + '/plugin/bootstrap-typeahead/bootstrap3-typeahead.js'], function () {
                        callback.call();
                    });
                } else if (type === 'bootstrap-select') {
                    require([Const.init.baseResourcePath + '/plugin/bootstrap-select/js/bootstrap-select.js'], function () {
                        callback.call();
                    });
                } else if (type === 'ztree') {
                    require(['css!' + Const.init.baseResourcePath + '/plugin/ztree/zTreeStyle.css', Const.init.baseResourcePath + '/plugin/ztree/jquery.ztree.core.min.js'], function () {
                        require([Const.init.baseResourcePath + '/plugin/ztree/jquery.ztree.excheck.min.js'], function () {
                            callback.call(null);
                        });
                    });
                } else if (type === 'summernote') {
                    require([Const.init.baseResourcePath + '/plugin/summernote/summernote.js'], function () {
                        require([Const.init.baseResourcePath + '/plugin/summernote/lang/summernote-zh-CN.min.js', 'css!' + Const.init.baseResourcePath + '/plugin/summernote/summernote.css'], function () {
                            callback.call();
                        });
                    });
                } else if (type === 'echarts') {
                    require([Const.init.baseResourcePath + '/plugin/echarts/echarts.min.js'], function (echarts) {
                        callback.call(null, echarts);
                    });
                } else if (type === 'multiselect') {
                    require(['css!' + Const.init.baseResourcePath + '/plugin/multiselect/css/style.css', Const.init.baseResourcePath + '/plugin/multiselect/js/multiselect.js'], function () {
                        callback.call(null);
                    });
                } else if (type === 'switch') {
                    require(['css!' + Const.init.baseResourcePath + '/plugin/bootstrap-switch/css/bootstrap-switch.min.css', Const.init.baseResourcePath + '/plugin/bootstrap-switch/js/bootstrap-switch.min.js'], function () {
                        callback.call(null);
                    });
                } else if (type === 'lodop') {
                    require(['http://localhost:8000/CLodopfuncs.js'], function () {
                        callback.call(null);
                    });
                } else if (type === 'qwebchannel') {
                    require([Const.init.baseResourcePath + '/plugin/websocket/qwebchannel.js'], function () {
                        callback.call(null);
                    });
                }
            }
        ...
        init: function(from) {
            if (!Const.inited) {
                ...
                Const.chart = this.mix(Const.chart, __Const.chart, true);
                var util = this;
                $.ajaxSetup ({
                    cache: false
                });
